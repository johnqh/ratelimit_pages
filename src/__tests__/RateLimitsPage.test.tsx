  ...
  UsageDashboard: vi.fn(
    ({
      ...
      <div data-testid='usage-dashboard'>
        ...
        {usageBars.map(
          (bar: { label: string; used: number; limit: number }, i: number) => (
      ...
      {tiers.map(
        (
          ...
          <div key={i} data-testid={`tier-${i}`}>
            ...
            <span data-testid={`tier-${i}-current`}>
              {tier.isCurrent ? 'yes' : 'no'}
            </span>
...
describe('RateLimitsPage', () => {
  ...
  describe('rendering states', () => {
    ...
    it('should render error state when error exists and no config', () => {
      ...
      expect(
        screen.getByText(/Failed to load rate limits/)
      ).toBeInTheDocument();
  ...
  describe('usage data display', () => {
    it('should display correct usage values for each period', () => {
      ...
      // Hourly
      expect(screen.getByTestId('usage-bar-0-label')).toHaveTextContent(
        'Hourly'
      );
      ...
      // Daily
      expect(screen.getByTestId('usage-bar-1-label')).toHaveTextContent(
        'Daily'
      );
      ...
      // Monthly
      expect(screen.getByTestId('usage-bar-2-label')).toHaveTextContent(
        'Monthly'
      );
      expect(screen.getByTestId('usage-bar-2-used')).toHaveTextContent('5000');
      expect(screen.getByTestId('usage-bar-2-limit')).toHaveTextContent(
        '10000'
      );
    });
    ...
    it('should pass usage title label to UsageDashboard', () => {
      ...
      expect(screen.getByTestId('usage-title')).toHaveTextContent(
        'Current Usage'
      );
    });
  });
